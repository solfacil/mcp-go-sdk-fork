...
func (ss *ServerSession) handle(ctx context.Context, req *jsonrpc.Request) (any, error) {
	...
	default:
		if !initialized && !validatedMeta.usesNewProtocol && req.IsCall() {
			// solfacil-patch: 'initialize' is dispatched sync but subsequent
			// calls are dispatched async (jsonrpc2.Async below) and, over the
			// streamable HTTP transport, each JSON-RPC request arrives as its
			// own HTTP POST handled by its own goroutine with no ordering
			// guarantee between requests of the same session. A client that
			// sends the next call immediately after 'initialize' without
			// awaiting its response can lose this race even though it did
			// everything right protocol-wise. Give the in-flight 'initialize'
			// a short grace window to finish setting state before rejecting.
			const initGracePeriod = 3 * time.Second
			const pollInterval = 5 * time.Millisecond
			deadline := time.Now().Add(initGracePeriod)
			for !initialized && time.Now().Before(deadline) {
				select {
				case <-ctx.Done():
					initialized = false
					deadline = time.Time{}
