...
func (ss *ServerSession) handle(ctx context.Context, req *jsonrpc.Request) (any, error) {
	...
	default:
		if !initialized && !validatedMeta.usesNewProtocol && req.IsCall() {
			// solfacil-patch: observed against a real client (Bifrost MCP
			// gateway, see https://github.com/maximhq/bifrost/issues/7202)
			// that some sessions never receive an 'initialize' request at
			// all before a call — no 'initialize' method is ever sent on
			// that connection, and no Mcp-Session-Id header is carried
			// either, so this is a brand new session as far as this server
			// is concerned. Rejecting outright makes such a client's tool
			// calls permanently fail. Since the client is otherwise trusted
			// (registered/authenticated at the transport layer) and this
			// server is only reachable from that trusted client, synthesize
			// a minimal InitializeParams so the call can proceed instead of
			// bouncing forever.
			ss.server.opts.Logger.Info("solfacil-patch: call received before initialize, synthesizing session state", "method", req.Method)
			ss.updateState(func(state *ServerSessionState) {
				if state.InitializeParams == nil {
					state.InitializeParams = &InitializeParams{
						ProtocolVersion: protocolVersion20250618,
						Capabilities:    &ClientCapabilities{},
						ClientInfo:      &Implementation{Name: "unknown-no-initialize", Version: "0.0.0"},
					}
				}
			})
			initialized = true
